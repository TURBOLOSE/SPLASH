...
class MUSCL_HLLD : public adiabatic
{


public:
    MUSCL_HLLD(SurfaceMesh mesh, std::vector<StateVec> U_in, double gam, double omega_ns_i, bool accretion_on_i, size_t threads)
        :adiabatic(mesh, U_in, gam, omega_ns_i,accretion_on_i, threads){
            if(!mag_field_on)
            {
                std::cout<<"HLLD should be used with magnetic field, check mag_field_on in parameters.json \n";
                stop_check=true;
                exit(1);
            }}
...
protected:
    ...
    { // returns vector F* or G*
        ...
        vector3d<double> vel_L, vel_R, l_vec, nxR, edge_center, B_L, B_R;
        ...
        B_L[0] = u_L[5]; B_L[1] = u_L[6]; B_L[2] = u_L[7];
        ...
        double p_tot_L = p_L + B_n_L * B_n_L / (2.0 * mu0);
        double p_tot_R = p_R + B_n_R * B_n_R / (2.0 * mu0);

        double V_L = dot_product(edge_normals[n_face][n_edge], vel_L);
        double V_R = dot_product(edge_normals[n_face][n_edge], vel_R);

        S_star = (p_tot_R - p_tot_L + rho_L * V_L * (S_L - V_L) -rho_R * V_R *(S_R - V_R)) /
                 (rho_L * (S_L - V_L) - rho_R * (S_R - V_R));
        
        if(std::isnan(S_star)||std::isinf(S_star))
        S_star = (V_L+V_R)/2;

        double p_tot_star = p_tot_L + rho_L * (S_L - V_L) * (S_star - V_L);

        // HLLD intermediate states
        StateVec U_L_star, U_R_star;
        
        double S_L_star = S_star - std::abs(dot_product(B_L, edge_normals[n_face][n_edge])) / std::sqrt(rho_L * (S_L - V_L) / (S_L - S_star));
        double S_R_star = S_star + std::abs(dot_product(B_R, edge_normals[n_face][n_edge])) / std::sqrt(rho_R * (S_R - V_R) / (S_R - S_star));

        // Note: For a proper HLLD solver in 3D geometry you need a full 8-wave U_L_star, U_L_star_star, U_R_star_star, U_R_star reconstruction.
        // We will approximate this by defining the F_L_star and F_R_star based on the single S_star first, which essentially represents HLLC for MHD.
        // To precisely implement HLLD, we need the 4 intermediate states. Let's do a simplified HLLC-like formulation for the moment based on the HLLC code provided.

        D[4] = S_star;

        double P_LR = (p_tot_L + p_tot_R + u_L[0] * (S_L - dot_product(edge_normals[n_face][n_edge], vel_L)) * (S_star - dot_product(edge_normals[n_face][n_edge], vel_L)) + u_R[0] * (S_R - dot_product(edge_normals[n_face][n_edge], vel_R)) * (S_star - dot_product(edge_normals[n_face][n_edge], vel_R))) / 2;
